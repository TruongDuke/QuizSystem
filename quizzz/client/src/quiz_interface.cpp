...
void manageQuestionsMenu(int sock, int quizId) {
    while (true) {
        ...
        if (choice == 1) {
            ...
            std::cin >> correct;
            std::string msg = "ADD_QUESTION|" + std::to_string(quizId) + "|" + q + "|" +
                o1 + "|" + o2 + "|" + o3 + "|" + o4 + "|" + std::to_string(correct);
            sendLine(sock, msg);
            std::cout << "Server: " << recvLine(sock) << std::endl;
        } else if (choice == 3) {
            int qid, correct;
            ...
            std::cin >> correct;
            std::string msg = "EDIT_QUESTION|" + std::to_string(qid) + "|" + q + "|" +
                o1 + "|" + o2 + "|" + o3 + "|" + o4 + "|" + std::to_string(correct);
            sendLine(sock, msg);
            std::cout << "Server: " << recvLine(sock) << std::endl;
        } else if (choice == 4) {
...
void teacherMenu(int sock) {
    while (true) {
        ...
        if (choice == 1) {
            ...
            std::cin >> timeLimit;
            std::string msg = "ADD_QUIZ|" + title + "|" + desc + "|" +
            ...
            std::cin >> timeLimit;
            std::string msg = "EDIT_QUIZ|" + std::to_string(quizId) + "|" +
                title + "|" + desc + "|" + std::to_string(count) + "|" + std::to_string(timeLimit);
            sendLine(sock, msg);
            std::cout << "Server: " << recvLine(sock) << std::endl;
        } else if (choice == 4) {
            int quizId;
