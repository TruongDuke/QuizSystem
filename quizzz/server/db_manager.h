...
/* --- CÁC STRUCT DỮ LIỆU ĐỂ GIAO TIẾP --- */

// Thông tin cơ bản của một bài Quiz
struct QuizInfo {
    int id;
    std::string title;
    std::string description;
    int timeLimit; // Giới hạn thời gian (giây)
};

// Thông tin một câu trả lời (lựa chọn)
struct AnswerInfo {
    int id;
    std::string text;
};

// Thông tin đầy đủ của một câu hỏi (bao gồm các lựa chọn)
struct QuestionInfo {
    int id;
    std::string text;
